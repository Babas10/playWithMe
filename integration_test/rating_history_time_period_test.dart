...
void main() {
  ...
  group('Rating History Time Period Filtering', () {
    test(
      'getRatingHistoryByPeriod returns entries within time period',
      () async {
        // 1. Create test user
    ...
    test(
      'getRatingHistoryByPeriod returns empty list when no entries in period',
      () async {
        // 1. Create test user
        final user = await FirebaseEmulatorHelper.createCompleteTestUser(
          email: 'player@test.com',
    ...
    test(
      'getRatingHistoryByPeriod returns entries ordered by timestamp descending',
      () async {
        // 1. Create test user
        final user = await FirebaseEmulatorHelper.createCompleteTestUser(
          email: 'player@test.com',
