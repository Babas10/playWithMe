...
void main() {
  ...
  group('Friend Request Acceptance Flow', () {
    test('User can accept friend request and both become friends', () async {
      ...
      final friendsResult = await getFriendsCallable.call({
        'userId': user2.uid,
      });
      ...
      expect(
        friendsResult.data['friends'][0]['email'],
        equals('user1@test.com'),
      );
      ...
      final friendsResult2 = await getFriendsCallable.call({
        'userId': user1.uid,
      });
      ...
      expect(
        friendsResult2.data['friends'][0]['email'],
        equals('user2@test.com'),
      );
    });

    test('Accepted request no longer appears in pending requests', () async {
      // 1. Create two test users
    ...
    test('Search result shows users are friends after acceptance', () async {
      // 1. Create two test users
      await FirebaseEmulatorHelper.createCompleteTestUser(
        email: 'user1@test.com',
        password: 'password123',
      ...
      final searchResult = await searchCallable.call({
        'email': 'user2@test.com',
      });
    ...
    test('Users cannot send new request to existing friends', () async {
    ...
    test('Only recipient can accept friend request', () async {
      // 1. Create two test users
      final user1 = await FirebaseEmulatorHelper.createCompleteTestUser(
        email: 'user1@test.com',
        password: 'password123',
