...
void main() {
  ...
  group('Friend Request Decline Flow', () {
    test('User can decline friend request', () async {
    ...
    test('Declined request no longer appears in pending requests', () async {
      // 1. Create two test users
      await FirebaseEmulatorHelper.createCompleteTestUser(
        email: 'user1@test.com',
        password: 'password123',
    ...
    test('User can send new request after previous was declined', () async {
      // 1. Create two test users
      await FirebaseEmulatorHelper.createCompleteTestUser(
        email: 'user1@test.com',
        password: 'password123',
      ...
      final firstRequest = await sendRequestCallable.call({
        'targetUserId': user2.uid,
      });
      ...
      final secondRequest = await sendRequestCallable.call({
        'targetUserId': user2.uid,
      });
    ...
    test('Initiator can cancel their own sent request', () async {
      // 1. Create two test users
    ...
    test('Search shows no pending request after decline', () async {
      // 1. Create two test users
      await FirebaseEmulatorHelper.createCompleteTestUser(
        email: 'user1@test.com',
        password: 'password123',
      ...
      final searchBefore = await searchCallable.call({
        'email': 'user2@test.com',
      });
      ...
      final searchAfter = await searchCallable.call({
        'email': 'user2@test.com',
      });
